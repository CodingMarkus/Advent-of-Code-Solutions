...
# Run as:
# cat advent_12_sample.txt | sh advent_12.sh
# cat advent_12_input.txt  | sh advent_12.sh
...
canGoLeft()
{
	...
	getHeight "$1" "$2" || h1=$?
	getHeight $(( $1 - 1 )) "$2" || h2=$?
	[ $(( h2 > h1 ? h2 - h1 : 0 )) -le 1 ] && return $true
	return $false
}
...
do
	...
	do
		...
		if [ "$xpos" -eq $destX ] && [ "$ypos" -eq $destY ]
		then
			echo "Steps: $steps"
			exit 0
		fi
